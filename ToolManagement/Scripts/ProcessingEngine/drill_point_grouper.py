    ...
    def group_drilling_points(self, data: dict) -> tuple[bool, str, dict[str, Any]]:
        ...
        if not data or "drill_points" not in data:
            return ErrorHandler.from_exception(
                ValidationError(
                    message="No drill points provided for grouping", severity=ErrorSeverity.ERROR
                )
            )
        ...
        try:
            ...
            if not drill_points:
                return ErrorHandler.from_exception(
                    ValidationError(
                        message="Empty drill points list for grouping", severity=ErrorSeverity.ERROR
                    )
                )
            ...
            for point in drill_points:
                ...
                if "diameter" not in point:
                    return ErrorHandler.from_exception(
                        ValidationError(
                            message="Drill point missing diameter", severity=ErrorSeverity.ERROR
                        )
                    )
            ...
            self.logger.error(f"Error grouping drill points: {e!s}")
            return ErrorHandler.from_exception(
                ValidationError(
                    message=f"Failed to group drill points: {e!s}", severity=ErrorSeverity.ERROR
                )
            )
...
if __name__ == "__main__":
    ...
    test_drill_points = [
        ...
        {"position": (200, 100, 0), "extrusion_vector": (0, 0, 1), "diameter": 8.0},
        {"position": (300, 150, 0), "extrusion_vector": (1, 0, 0), "diameter": 8.0},
        {"position": (400, 200, 0), "extrusion_vector": (1, 0, 0), "diameter": 10.0},
    ]

    test_data = {"drill_points": test_drill_points}
