...
class MachinePositioner:
    ...
    def position_for_top_left_machine(self, data: dict) -> tuple[bool, str, dict[str, Any]]:
        ...
        if not data:
            return ErrorHandler.from_exception(
                ValidationError(
                    message="No data provided for positioning", severity=ErrorSeverity.ERROR
                )
            )
        ...
        try:
            ...
            success_msg = (
                f"Positioned workpiece with offset ({rounded_offset_x}, {rounded_offset_y}) "
            ...
            self.logger.error(f"Error in workpiece positioning: {e!s}")
            return ErrorHandler.from_exception(
                ValidationError(
                    message=f"Failed to position workpiece: {e!s}", severity=ErrorSeverity.ERROR
                )
            )
    ...
    def _apply_offset_to_coordinates(
        self, coordinates: tuple[float, float, float], offset: tuple[float, float]
...
if __name__ == "__main__":
    ...
    test_drill_points = [
        {"position": (50, 100, 0), "extrusion_vector": (0, 0, 1), "diameter": 8.0},
        {"position": (200, 400, 0), "extrusion_vector": (1, 0, 0), "diameter": 10.0},
    ]
