...
class WorkpieceRotator:
        ...
        try:
            ...
            if 'position' not in point:
                return ErrorHandler.from_exception(
                    ValidationError(
                        message="Point missing 'position' attribute",
            ...
            return ErrorHandler.create_success_response(
                ...
                data={
                    ...
                    'rotated_position': point['position']
                }
            )
            
        except Exception as e:
            # Log and return error
            self.logger.error(f"Error rotating point: {str(e)}")
            return ErrorHandler.from_exception(
                ValidationError(
                    message=f"Failed to rotate point: {str(e)}",
                    severity=ErrorSeverity.ERROR
                )
            )
    ...
    def transform_drilling_data(self, data: Dict) -> Tuple[bool, str, Dict[str, Any]]:
        ...
        try:
            ...
            success_msg = (f"Transformed workpiece to {orientation} orientation and "
            ...
            return ErrorHandler.from_exception(
                ValidationError(
                    message=f"Failed to transform drilling data: {str(e)}",
                    severity=ErrorSeverity.ERROR
                )
            )
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print(f"Dimensions: {result['original_dimensions']['width']}x"
        ...
        print(f"Rotated dimensions: {result['rotated_dimensions']['width']}x"
