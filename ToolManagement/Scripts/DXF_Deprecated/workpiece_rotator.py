...
class WorkpieceRotator:
    ...
    def determine_edge_from_vector(self, vector: Tuple[float, float, float]) -> str:
        ...
        When a drill moves in a certain direction (given by the extrusion vector),
        it's actually drilling on the opposite edge of the workpiece.
        ...
        if abs(vz) > 0.9:  # Tolerance for numerical imprecision
            ...
            # Invert X direction logic:
            # When drilling in -X direction (leftward), we're drilling the RIGHT edge
            # When drilling in +X direction (rightward), we're drilling the LEFT edge
            return "LEFT" if vx > 0 else "RIGHT"
        elif abs(vy) > 0.9:
            # Invert Y direction logic:
            # When drilling in -Y direction (backward), we're drilling the FRONT edge
            # When drilling in +Y direction (forward), we're drilling the BACK edge
            return "BACK" if vy > 0 else "FRONT"
        else:
            return "UNKNOWN"
