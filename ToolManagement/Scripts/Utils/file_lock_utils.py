...
class FileLock:
    ...
    def __init__(self, file_path: Union[str, Path], timeout: float = None):
        """
        ...
        try:
            ...
            if platform.system() == 'Windows':
                try:
                    ...
                    return True, f"File is locked by another process: {str(e)}"
            ...
            self.logger.error(f"Error checking file lock: {str(e)}")
            # If we can't check, assume it's not locked
            return False, f"Error checking lock: {str(e)}"
    ...
    def acquire(self) -> bool:
        ...
        try:
            ...
            if self.lock_path.exists():
                ...
                if self._is_stale_lock():
                    ...
                    try:
                        ...
                        self.logger.error(f"Failed to remove stale lock: {self.lock_path} - {str(e)}")
                        return False
                else:
                    # Lock is still valid
            ...
            try:
                ...
                self.logger.error(f"Failed to create lock file: {self.lock_path} - {str(e)}")
                return False
    ...
    def _is_stale_lock(self) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Error checking stale lock: {str(e)}")
