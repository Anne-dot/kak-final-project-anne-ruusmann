...
from .error_utils import ErrorHandler, FileError, ErrorSeverity, ErrorCategory
...
class BaseFileLoader(ABC):
    ...
    def __init__(self, allowed_extensions: List[str] = None, description: str = "File"):
        ...
        self.logger.info(f"{self.__class__.__name__} initialized with allowed extensions: {self.allowed_extensions}")
    
    def validate_file(self, file_path: Union[str, Path]) -> Tuple[bool, str, Dict[str, Any]]:
        """
        ...
        if self.allowed_extensions and not any(file_path.name.lower().endswith(ext.lower()) for ext in self.allowed_extensions):
        ...
        return ErrorHandler.create_success_response(
            message=f"{self.description} file is valid",
            data={"file_path": str(file_path)}
        )
    
    def select_file(self, test_data_dir: str = None, title: str = None, filetypes: List[Tuple[str, str]] = None) -> Optional[str]:
        """
        ...
        if filetypes is None and self.allowed_extensions:
            filetypes = [(f"{self.description} files", "*" + ext) for ext in self.allowed_extensions]
        ...
        # Linux/Other: Show file list in terminal
            ...
            if self.allowed_extensions:
                ...
                for ext in self.allowed_extensions:
                    matching_files.extend([f for f in os.listdir(test_data_dir) 
    ...
    @abstractmethod
    def load_file(self, file_path: Optional[Union[str, Path]] = None) -> Tuple[bool, str, Dict[str, Any]]:
        """
