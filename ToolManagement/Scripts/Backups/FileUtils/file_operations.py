...
def create_lock_file(
    lock_file_path: str, content_dict: dict[str, Any]
    ...
    try:
        ...
        return ErrorHandler.create_success_response(
            ...
            data={"lock_file_path": lock_file_path, "content_keys": list(content_dict.keys())},
        )
        ...
        error_msg = f"Error creating lock file {lock_file_path}: {e!s}"
...
def remove_file_safely(
    ...
    if not os.path.exists(file_path):
        ...
        return ErrorHandler.create_success_response(
            message="File does not exist, nothing to remove", data={"file_path": file_path}
        )
    ...
    for attempt in range(max_retries):
        try:
            ...
            return ErrorHandler.create_success_response(
                ...
                data={"file_path": file_path, "attempts_required": attempt + 1},
            )
            ...
            if attempt == max_retries - 1:
                # Last attempt failed
                error_msg = (
                    f"Permission denied removing file {file_path} after {max_retries} attempts"
                )
                logger.error(error_msg)
                return ErrorHandler.from_exception(
                    FileError(
            ...
            logger.info(
                f"Retrying file removal in {actual_delay:.2f} seconds (attempt {attempt + 1}/{max_retries})"
            )
            ...
            if attempt == max_retries - 1:
                # Last attempt failed
                error_msg = f"Failed to remove file {file_path} after {max_retries} attempts: {e!s}"
                log_exception(logger, error_msg)
                return ErrorHandler.from_exception(
                    FileError(
                        message=error_msg,
...
def backup_file(
    file_path: str, backup_dir: str | None = None, max_backups: int = 10
) -> tuple[bool, str, dict[str, Any]]:
    """
    ...
    try:
        ...
        error_msg = f"Error creating backup of {file_path}: {e!s}"
        log_exception(logger, error_msg)
        return ErrorHandler.from_exception(
            FileError(
                message=error_msg,
...
def prune_old_backups(
    backup_dir: str, base_name: str, extension: str, max_backups: int
) -> tuple[bool, str, dict[str, Any]]:
    """
    ...
    try:
        logger.info(
            f"Pruning old backups in {backup_dir} for {base_name}{extension}, keeping {max_backups}"
        )
        ...
        if len(backup_files) > max_backups:
            ...
            for file_path, _ in files_to_remove:
                ...
                if success:
                    ...
                    failed_removals.append({"file": os.path.basename(file_path), "error": message})

            if failed_removals:
                warning_msg = (
                    f"Partially pruned backups: removed {removed_count} of {len(files_to_remove)}"
                )
        ...
        logger.info(
            f"No pruning needed, only {len(backup_files)} backups exist (max is {max_backups})"
        )
        return ErrorHandler.create_success_response(
        ...
        error_msg = f"Error pruning old backups: {e!s}"
        log_exception(logger, error_msg)
        return ErrorHandler.from_exception(
            FileError(
                message=error_msg,
