...
class CSVEditor:
    ...
    def _create_controls(self, parent):
        ...
        ttk.Button(control_frame, text="Save", command=self._save_tool_data_to_csv).pack(
            side=tk.LEFT, padx=5
        )
        ttk.Button(control_frame, text="Reload", command=self._reload_tool_data_from_disk).pack(
            side=tk.LEFT, padx=5
        )
    ...
    def _create_startup_backup(self):
        ...
        try:
            success, message, details = self.backup_manager.create_backup(self.csv_path)
            if success:
            ...
            self._show_error(f"Backup error: {e!s}")
    ...
    def _load_csv_tool_data(self):
        ...
        try:
            ...
            self._show_error(f"Load error: {e!s}")
    ...
    def _save_tool_data_to_csv(self):
        ...
        try:
            ...
            self._show_error(f"Save error: {e!s}")
