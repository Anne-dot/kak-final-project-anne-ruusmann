...
class VisualCoordinateTranslator:
    ...
    def translate_coordinates(self, drill_points: List[Dict], workpiece: Dict) -> Tuple[bool, str, Dict[str, Any]]:
        """
        ...
        try:
            ...
            message = (f"Translated {total_translated} horizontal drill points "
            ...
            self.logger.error(f"Error in coordinate translation: {str(e)}")
            return ErrorHandler.from_exception(
                ValidationError(
                    message=f"Failed to translate coordinates: {str(e)}",
    ...
    def _validate_workpiece(self, workpiece: Dict) -> Tuple[bool, str, Dict]:
        ...
        if not all(field in workpiece for field in required_fields):
            return ErrorHandler.from_exception(
                ValidationError(
                    message="Workpiece missing required dimensions",
                    severity=ErrorSeverity.ERROR
                )
            )
        ...
        try:
            ...
            return ErrorHandler.from_exception(
                ValidationError(
                    message=f"Invalid workpiece dimensions: {str(e)}",
                    severity=ErrorSeverity.ERROR
                )
            )
