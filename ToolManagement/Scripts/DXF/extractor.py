    ...
    def extract(self, document) -> tuple[bool, str, dict[str, Any]]:
        ...
        try:
            ...
            for entity in modelspace:
                ...
                is_potential_drill = (
                    hasattr(entity, "dxftype")
                    and hasattr(entity.dxf, "layer")
                    and ("DRILL" in entity.dxf.layer or "DRILLING" in entity.dxf.layer)
                )
            ...
            if skipped_points > 0:
                message = (
                    f"Extracted {len(drill_points)} drill points. "
            ...
            self.logger.error(f"Error in drilling extraction: {e!s}")
            return ErrorHandler.from_exception(
                ValidationError(
                    message=f"Failed to extract drilling operations: {e!s}",
                    severity=ErrorSeverity.ERROR,
                )
            )

    def _extract_from_entity(self, entity) -> dict[str, Any] | None:
        """
        ...
        try:
            ...
            self.logger.warning(f"Error extracting from entity: {e!s}")
            return None

    def _extract_depth_from_layer(self, layer_name: str) -> float | None:
        """
        ...
        try:
            ...
            self.logger.debug(f"Could not extract depth from layer '{layer_name}': {e!s}")
            return None

    def _extract_diameter_from_layer(self, layer_name: str) -> float | None:
        """
        ...
        try:
            ...
            self.logger.debug(f"Could not extract diameter from layer '{layer_name}': {e!s}")
            return None

    def _extract_from_circle(self, circle, layer) -> dict[str, Any] | None:
        """
        ...
        try:
            ...
            if diameter_from_layer is not None and diameter_from_entity > 0:
                ...
                diff_percent = (
                    abs(diameter_from_entity - diameter_from_layer) / diameter_from_layer * 100
                )

                # Store mismatch info
                diameter_mismatch = {"percent": diff_percent, "is_significant": diff_percent > 2}
            ...
            self.logger.warning(f"Error extracting from circle: {e!s}")
            return None

    def _extract_from_point(self, entity, layer) -> dict[str, Any] | None:
        """
        ...
        """
        self.logger.debug("POINT entity extraction not yet implemented - skipping")
        return None

    def _extract_from_insert(self, entity, layer) -> dict[str, Any] | None:
        """
        ...
        Returns:
            dict or None: Dictionary with drill point properties (currently None)
        """
        self.logger.debug("INSERT entity extraction not yet implemented - skipping")
        return None
...
class WorkpieceExtractor:
    ...
    def extract(self, document) -> tuple[bool, str, dict[str, Any]]:
        ...
        try:
            ...
            if not workpiece_data:
                ...
                return ErrorHandler.from_exception(
                    ValidationError(
                        message="No workpiece boundary found. Workpiece extraction requires a closed polyline on a 'PANEL_*' or 'OUTLINE_*' layer.",
                        severity=ErrorSeverity.ERROR,
                    )
                )
            ...
            # Log and return error
            self.logger.error(f"Error extracting workpiece: {e!s}")
            return ErrorHandler.from_exception(
                ValidationError(
                    message=f"Failed to extract workpiece boundary: {e!s}",
                    severity=ErrorSeverity.ERROR,
                )
            )

    def _extract_from_polylines(self, modelspace) -> dict[str, Any] | None:
        """
        ...
        if workpiece_polyline.dxftype() == "LWPOLYLINE":
            ...
            corner_points = [
                (vertex.dxf.location[0], vertex.dxf.location[1], 0)
        ...
        if len(corner_points) < 3:
            self.logger.error(
                f"Invalid workpiece: Polyline on layer '{layer}' has fewer than 3 corner points"
            )
            return None
    ...
    def _extract_thickness_from_layer(self, layer_name: str) -> float | None:
        """
        ...
        try:
            ...
            self.logger.debug(f"Could not extract thickness from layer '{layer_name}': {e!s}")
            return None
    ...
    def process(self, document) -> tuple[bool, str, dict[str, Any]]:
        ...
        try:
            ...
            if not drill_success or not drill_result.get("drill_points", []):
                self.logger.error(
                    f"Drill point extraction failed or no points found: {drill_message}"
                )
                return ErrorHandler.from_exception(
                    ValidationError(
            ...
            # Log and return error
            self.logger.error(f"Error in DXF extraction: {e!s}")
            return ErrorHandler.from_exception(
                ValidationError(
                    message=f"Failed to process DXF document: {e!s}", severity=ErrorSeverity.ERROR
                )
            )
