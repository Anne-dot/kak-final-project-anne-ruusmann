...
def method1_rename_test(file_path):
    ...
    try:
        ...
        debug_print(f"Unexpected exception: {e!s}", "ERROR")
    ...
    try:
        ...
        with open(file_path, "r+") as f:
            ...
            try:
                ...
                debug_print(f"Error during Windows API call: {e!s}", "ERROR")
                debug_print("RESULT: Assuming file is locked due to API error", "WARNING")
                return True  # Assume locked on any exception
    except PermissionError as e:
        debug_print(f"PermissionError occurred: {e}", "ERROR")
        debug_print(
            "RESULT: File IS locked (exclusive open failed with PermissionError)", "WARNING"
        )
        return True
    except OSError as e:
...
def is_file_locked(file_path):
    ...
    debug_print("File exists, proceeding with lock detection tests", "INFO")
    ...
    debug_print(
        f"Method 1 result: {'LOCKED' if locked_1 else 'NOT LOCKED'}",
    ...
    debug_print(
        f"Method 2 result: {'LOCKED' if locked_2 else 'NOT LOCKED'}",
    ...
    debug_print(
        f"Method 3 result: {'LOCKED' if locked_3 else 'NOT LOCKED'}",
    ...
    debug_print(
        f"Method 4 result: {'LOCKED' if locked_4 else 'NOT LOCKED'}",
    ...
    debug_print(
        f"Method 5 result: {'LOCKED' if locked_5 else 'NOT LOCKED'}",
    ...
    methods_locked = []
    if locked_1:
        methods_locked.append("Rename")
    if locked_2:
        methods_locked.append("Exclusive Open")
    if locked_3:
        methods_locked.append("Directory Write")
    if locked_4:
        methods_locked.append("Copy")
    if locked_5:
        methods_locked.append("Windows API")

    debug_print("=" * 40, "INFO")
    if result:
        debug_print("FINAL RESULT: FILE IS LOCKED", "WARNING")
        if methods_locked:
            debug_print(f"Lock detected by methods: {', '.join(methods_locked)}", "WARNING")
            # Give more information about which app might be locking
            debug_print(
                "File is likely open in a program like Notepad, Wordpad, or another text editor",
                "WARNING",
            )
            debug_print("Please close the file in all applications before continuing", "WARNING")
        else:
            debug_print(
                "Lock detection succeeded but no specific method identified the lock", "ERROR"
            )
    else:
        debug_print("FINAL RESULT: FILE IS NOT LOCKED", "SUCCESS")
        debug_print("All detection methods indicate file is accessible", "SUCCESS")
    debug_print("=" * 40, "INFO")
...
def method5_mswindows_test(file_path):
    ...
    try:
        ...
        debug_print(f"Exception during Windows API test: {e!s}", "ERROR")
...
if __name__ == "__main__":
    ...
    if not is_locked:
        ...
        if locked_4 or locked_5:
            debug_print(
                "\nWARNING: Basic methods did not detect lock, but additional methods did!",
                "WARNING",
            )
            is_locked = True
