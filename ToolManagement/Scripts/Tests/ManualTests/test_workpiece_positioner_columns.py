#!/usr/bin/env python3
"""
Test script for workpiece positioning functionality with columnar output.

This script demonstrates how the workpiece positioner moves drilling points
to different positions in the machine coordinate system with a clear side-by-side
comparison of coordinates before and after positioning.
"""
...
def store_original_positions(points):
    """Store the original positions before positioning for comparison."""
    for point in points:
        point.original_position = point.machine_position


def print_points_comparison(points, title="Points Comparison", show_header=True):
    """Print points with their original and positioned coordinates side by side."""
    if show_header:
        print(f"\n{title}")
        print("=" * len(title))
        
        # Print header row
        print(f"{'ID':<3} {'EDGE':<10} {'ORIGINAL (X, Y, Z)':<20} {'POSITIONED (X, Y, Z)':<20} {'OFFSET':<15}")
        print("-" * 70)
    
    for point in points:
        orig_x, orig_y, orig_z = point.original_position
        pos_x, pos_y, pos_z = point.machine_position
        
        # Calculate applied offset for this point
        offset_x = pos_x - orig_x
        offset_y = pos_y - orig_y
        offset_z = pos_z - orig_z
        
        # Format for display
        original_pos = f"({orig_x:<6.1f}, {orig_y:<6.1f}, {orig_z:<4.1f})"
        positioned_pos = f"({pos_x:<6.1f}, {pos_y:<6.1f}, {pos_z:<4.1f})"
        offset = f"({offset_x:<+5.1f}, {offset_y:<+5.1f}, {offset_z:<+3.1f})"
        
        print(f"{point.id:<3} {point.edge:<10} {original_pos:<20} {positioned_pos:<20} {offset:<15}")
...
def test_positioning_from_orientation(width, height, current_orientation, target_position):
    ...
    store_original_positions(points)
    ...
    # Print positioned points with comparison to original positions
    print_points_comparison(points, f"Comparison of points before and after positioning to {target_position}")
...
def test_all_positionings():
    """Test all possible positioning combinations with columnar output."""
    print("=== WORKPIECE POSITIONING TESTS WITH COLUMNAR OUTPUT ===")
    print("This script tests the positioning of workpieces in the machine coordinate system.")
    print("Original and positioned coordinates are shown side by side for easy comparison.")
