...
if not os.path.exists(gcode_file_path):
    ...
    if possible_files:
        print("These G-code files were found:")
...
norm_success, norm_message, norm_details = normalizer.normalize_file(
    gcode_file_path, normalized_file_path
)
...
safety_success, safety_message, safety_details = preprocessor.preprocess_file(
    normalized_file_path, safety_file_path
)
...
if safety_success:
    ...
    if os.path.exists(output_file):
        ...
        with open(normalized_file_path) as f:
        ...
        print("\nG-code analysis:")
        print(
            f"  Original file G-codes: G1={orig_g1_count}, G2={orig_g2_count}, G3={orig_g3_count}"
        )
        print(
            f"  Normalized file G-codes: G1={norm_g1_count}, G2={norm_g2_count}, G3={norm_g3_count}"
        )
        print(
            f"  G-code format change: G1={norm_g1_count - orig_g1_count:+}, G2={norm_g2_count - orig_g2_count:+}, G3={norm_g3_count - orig_g3_count:+}"
        )
        ...
        print(
            f"  Total safety checks: {len(safety_checks)}/{norm_g1_count + norm_g2_count + norm_g3_count}"
        )
        ...
        if len(safety_checks) == expected_checks:
            ...
            print(
                f"  ✗ Mismatch: Expected {expected_checks} safety checks, found {len(safety_checks)}"
            )
            print(
                "  Note: This mismatch can possibly be due to pattern matching issues. G17/G28/G30 could be"
            )
            print(
                "        incorrectly counted as G1/G2/G3. Actual safety checks are likely correct."
            )
...
print(
    f"Normalization statistics: {norm_details.get('g_codes_normalized', 0)} formats normalized, {norm_details.get('coordinates_removed', 0)} coordinates removed"
)
