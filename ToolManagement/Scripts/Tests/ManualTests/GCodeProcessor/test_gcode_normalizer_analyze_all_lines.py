...
def compare_lines(original, normalized):
    ...
    if original.strip().startswith("#"):
        ...
        if comment_match and (
            comment_match.group(1) == "G1"
            or comment_match.group(1) == "G2"
            or comment_match.group(1) == "G3"
        ):
            reason = "Variable assignment with G-code reference in comment - normalized format in comment"
            return {
    ...
    if original == normalized and (
        re.search(r"G00", original)
        or re.search(r"G01", original)
        or re.search(r"G02", original)
        or re.search(r"G03", original)
    ):
...
def test_gcode_normalizer(gcode_file):
    ...
    output_file = (
        f"{os.path.splitext(input_basename)[0]}_normalized{os.path.splitext(input_basename)[1]}"
    )
    ...
    if success:
        ...
        line_mapping = {}
        orig_lines_with_n = [
            (i, line) for i, line in enumerate(lines) if re.search(r"^N\d+", line.strip())
        ]
        norm_lines_with_n = [
            (i, line)
            for i, line in enumerate(normalized_lines[norm_start:norm_end])
    ...
    if len(sys.argv) > 1:
        ...
        for ext in ["*.nc", "*.gcode"]:
            for file_path in test_data_dir.glob(ext):
                if not any(
                    suffix in file_path.name for suffix in ["_normalized", "_safe", "_test"]
                ):
                    gcode_files.append(str(file_path))
