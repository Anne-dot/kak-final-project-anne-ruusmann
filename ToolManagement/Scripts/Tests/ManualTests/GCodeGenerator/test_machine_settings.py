...
def test_workpiece_dimension_handling(automated=False) -> None:
    ...
    for name, dimensions in test_workpieces.items():
        ...
        print(
            f"Raw dimensions: width={dimensions['width']}, height={dimensions['height']}, thickness={dimensions['thickness']}"
        )

        # Generate header for this workpiece
        header = settings.get_default_gcode_header(
            dimensions, f"{name.lower().replace(' ', '_')}.nc"
        )
...
def test_coordinate_system_selection(automated=False) -> None:
    ...
    print(
        f"Small workpieces (height <= {threshold}mm) use {AppConfig.gcode.COORDINATE_SYSTEM_SMALL}"
    )
    print(
        f"Large workpieces (height > {threshold}mm) use {AppConfig.gcode.COORDINATE_SYSTEM_LARGE}"
    )
...
def test_vector_to_axis_mapping(automated=False) -> None:
    ...
    for vector in test_vectors:
        ...
        print(f"{vector!s:<20} {info['axis']:<7} {info['direction']:<11} {info['description']}")
