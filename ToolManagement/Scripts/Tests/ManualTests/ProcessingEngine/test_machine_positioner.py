...
def print_corner_points_comparison(
    original_corners: list[tuple], machine_corners: list[tuple]
) -> None:
    """Print a comparison table of original and machine corner points."""
    print("{:<10} {:<30} {:<30}".format("Corner", "Original Position", "Machine Position"))
    print("-" * 70)

    for i, (orig, mach) in enumerate(zip(original_corners, machine_corners, strict=False)):
        orig_str = f"({orig[0]:.1f}, {orig[1]:.1f}, {orig[2]:.1f})"
        mach_str = f"({mach[0]:.1f}, {mach[1]:.1f}, {mach[2]:.1f})"

        print("{:<10} {:<30} {:<30}".format(f"Corner {i}", orig_str, mach_str))
...
def print_drill_points_comparison(drill_points: list[dict]) -> None:
    ...
    for i, point in enumerate(drill_points, 1):
        ...
        print(f"{i:<4} {orig_str:<30} {mach_str:<30} {diameter:<15.2f} mm")
...
def create_test_drill_points() -> list[dict]:
    ...
    return [
        {"position": (100, 100, 0), "extrusion_vector": (0, 0, 1), "diameter": 8.0},
        {"position": (400, 200, 0), "extrusion_vector": (1, 0, 0), "diameter": 10.0},
        {"position": (-100, 150, 0), "extrusion_vector": (0, 1, 0), "diameter": 12.0},
    ]


def test_positioning_with_workpiece(
    workpiece: dict, drill_points: list[dict], rotation_count: int = 0
) -> None:
    """Test the positioning process with a given workpiece."""
    # Create data structure
    test_data = {"workpiece": workpiece, "drill_points": drill_points}
    ...
    print_corner_points_comparison(
        result["original_corner_points"], result["machine_corner_points"]
    )
