...
def create_test_summary(result):
    ...
    test_details = {
        "Parser initialization": 
            f"{status} {'Verified settings, extensions' if result.wasSuccessful() else 'Failed to verify settings'}",
        
        "File operations": {
            "Loading valid file": 
                f"{status} {'Successfully loads, returns document' if result.wasSuccessful() else 'Failed to load document'}",
            "Loading non-existent file": 
                f"{status} {'Proper error handling' if result.wasSuccessful() else 'Improper error handling'}",
            "Loading invalid DXF format": 
                f"{status} {'Returns appropriate error' if result.wasSuccessful() else 'Incorrect error response'}",
            "Loading empty file": 
                f"{status} {'Prevents empty file usage' if result.wasSuccessful() else 'Failed to handle empty file'}"
        },
        
        "Document operations": {
            "Extracting file info": 
                f"{status} {'Returns entity counts, types, layers' if result.wasSuccessful() else 'Failed to extract data'}",
            "Handling missing document": 
                f"{status} {'Gracefully handles errors' if result.wasSuccessful() else 'Improper error handling'}"
        }
    }
    ...
    output.append("")
    output.append(f"All validation checks {'passed' if result.wasSuccessful() else 'failed'} {status}")
    ...
    if not result.wasSuccessful():
        ...
        for idx, (test, error) in enumerate(result.failures + result.errors, 1):
            ...
            output.append(f"{idx}. {test_name}: {error.splitlines()[0] if error else 'Unknown error'}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--verbose", "-v", action="store_true", help="Show detailed test output and logs")
    parser.add_argument("--check-import", action="store_true", help="Only check if DXF parser can be imported")
    ...
    try:
        ...
        if args.check_import:
            parser = DXFParser()
            print(f"✓ DXFParser instance created successfully")
