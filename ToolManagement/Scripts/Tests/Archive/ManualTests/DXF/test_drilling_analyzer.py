...
This test runs the drilling analyzer on a DXF file and displays complete
results showing all drilling points to verify correct grouping and classification.
"""
...
def display_points_by_edge(points_by_edge):
...
def display_classified_points(drilling_points):
    """Display drilling points with their classification."""
    UIUtils.print_separator("Classified Drilling Points")
    
    vertical_points = []
    horizontal_points = []
    
    for point in drilling_points:
        drill_type = getattr(point, 'drill_type', 'unknown')
        edge = getattr(point, 'edge', 'UNKNOWN')
        
        if drill_type == 'vertical':
            vertical_points.append(point)
        elif drill_type == 'horizontal':
            horizontal_points.append(point)
    
    print(f"Total classified points: {len(drilling_points)}")
    print(f"Vertical points: {len(vertical_points)}")
    print(f"Horizontal points: {len(horizontal_points)}")
    
    # Show sample of each type
    if vertical_points:
        UIUtils.print_separator("Vertical Drilling Points (Sample)")
        for i, point in enumerate(vertical_points[:5]):  # Show first 5
            pos = point.position
            pos_str = f"({pos[0]:.1f}, {pos[1]:.1f}, {pos[2]:.1f})" if pos else "None"
            print(f"{i+1}. {pos_str}, D={point.diameter:.1f}mm, Depth={point.depth:.1f}mm, Edge={point.edge}")
        
        if len(vertical_points) > 5:
            print(f"...and {len(vertical_points) - 5} more vertical points")
    
    if horizontal_points:
        UIUtils.print_separator("Horizontal Drilling Points (Sample)")
        for i, point in enumerate(horizontal_points[:5]):  # Show first 5
            pos = point.position
            pos_str = f"({pos[0]:.1f}, {pos[1]:.1f}, {pos[2]:.1f})" if pos else "None"
            print(f"{i+1}. {pos_str}, D={point.diameter:.1f}mm, Depth={point.depth:.1f}mm, Edge={point.edge}")
        
        if len(horizontal_points) > 5:
            print(f"...and {len(horizontal_points) - 5} more horizontal points")

print("=== Drilling Analyzer OOP Test ===")
...
UIUtils.print_separator("Step 4: Classifying Drilling Points")
success, classified_points, classification_details = classifier.classify_points(drilling_info['points'])

if not success:
    print(f"Error classifying drilling points: {classification_details.get('error', 'Unknown error')}")
    UIUtils.keep_terminal_open("Test failed at drilling classification stage.")
    sys.exit(1)

# Get points by edge after classification
points_by_edge = classifier.get_points_by_edge(classified_points)

print(f"SUCCESS: Classified {classification_details['classified_count']} drilling points")

# Show initial summary
UIUtils.print_separator("Initial Results Summary")
stats = results['statistics']
print(f"Total points: {stats['total_points']}")
print(f"Tool groups: {stats['total_groups']}")
print(f"Vertical groups: {stats['vertical_groups']} ({stats['vertical_points']} points)")
print(f"Horizontal groups: {stats['horizontal_groups']} ({stats['horizontal_points']} points)")
...
while True:
    ...
    print("5. Show classified points")
    print("6. Exit test")
    ...
    if choice == "1":
        ...
        display_points_by_edge(points_by_edge)
    elif choice == "4":
        display_edge_summary(results, details)
    elif choice == "5":
        display_classified_points(classified_points)
    elif choice == "6":
        break
    else:
